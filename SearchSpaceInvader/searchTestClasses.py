...
class GraphSearchTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.graphText = testDict['graph']
        self.algorithm = testDict['algorithm']
        self.diagram = testDict['diagram']
        # TODO: check the anem of exactStateExpansionOrder
        self.exactStateExpansionOrder = testDict.get('exactStateExpansionOrder', 'True').lower() == "true"
    ...
    def getSolInfo(self, search):
        algorithm = getattr(search, self.algorithm)
        problem = GraphSearch(self.graphText)
        if self.heuristic != None:
            solution = algorithm(problem, self.heuristic)
        else:
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solution in gold_solution and (not self.exactStateExpansionOrder or expanded_states in gold_expanded_states):
...
class SpaceShipSearchTest(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(SpaceShipSearchTest, self).__init__(question, testDict)
    ...
    def getSolInfo(self, search, searchAgents):
        alg = getattr(search, self.algorithm)
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        start_state = spaceship.GameState()
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        grades.addMessage('PASS: %s' % self.path)
        grades.addMessage('\tspace invaders layout:\t\t%s' % self.layoutName)
...
def getStatesFromPath(start, path):
    ...
    for a in path:
        x,y = curr
        ((dx, dy), fire) = Actions.actionToVector(a)
        curr = (int(x + dx), int(y + dy))
        if fire:
            vis.append((int(x+dx), 1))
        vis.append(curr)
    return vis
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        gameState = spaceship.GameState()
        gameState.initialize(lay, 0)
        visited = getStatesFromPath(gameState.getSpaceShipPosition(), path)
        top, right = gameState.getWalls().height-2, gameState.getWalls().width-2
        missedCorners = [p for p in ((1,1), (right, 1)) if p not in visited]
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        search = moduleDict['search']
        searchAgents = moduleDict['searchAgents']
        game_state = spaceship.GameState()
        ...
        if heuristic_cost == 0:
            ...
            return False
        # cornerAdmissible
        print("heurisctic_cost is:", heuristic_cost)
        print("true_cost is:", true_cost)
        ...
        for i in range(0, len(heuristics) - 1):
            h0 = heuristics[i]
            h1 = heuristics[i+1]
            # print("h0, path, h1", h0, path[i], h1)
            # print("state[i], state[i+1]", states[i], states[i+1])
            # cornerConsistencyB
            if h0 - h1 > 1:
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        handle.write('cost: "%d"\n' % (len(solution)-2)) # here I am doing this -2 since to remove Fire action from costing.
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.close()
        return True
...
class CornerHeuristicSpaceShip(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(CornerHeuristicSpaceShip, self).__init__(question, testDict)
        self.layout_text = testDict['layout']
